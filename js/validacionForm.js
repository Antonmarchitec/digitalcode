...
function validarFormularioDetalle(){
    //para correo electronico
    let nameDetalle = document.querySelector("#nameDetalle").value
    let mailDetalle = document.querySelector("#mailDetalle").value

    document.querySelector("#textareaDetalle").focus()

    let expresionRegular = /\w+@\w+\.+[a-z]/

       if( nameDetalle === "" || mailDetalle === ""){
           //alert("rellene los espacios")
           Swal.fire({
               title:"<h2 class = 'title'> ¡Rellenar todos los campos! <h2>",    //tilte si se acepta codigo html
               //html:"RELLENAR TODOS LOS ESPACIOS",  //si se permite html
           ...
           return false
       }else if(!expresionRegular.test(mailDetalle)){
